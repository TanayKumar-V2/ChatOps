...
export default function App() {
  ...
  return <><div className="app-shell"><Sidebar activeRoom={activeRoom} rooms={rooms} userName={user?.name ?? "Your account"} avatarUrl={user?.avatarUrl} onRoomChange={setActiveRoom} onCreateRoom={() => setCreateRoomOpen(true)} onJoinRoom={() => setJoinRoomOpen(true)} onEditProfile={() => setProfileOpen(true)} onLogout={logout} />{activeRoomData ? <><ChatPanel roomId={activeRoomData.id} room={activeRoomData} currentUser={user} onStatsDelta={(delta) => updateRoomStats(activeRoomData.id, delta)} /><DetailsPanel room={activeRoomData} /></> : <main className="chat-panel empty-room"><div><div className="room-icon">#</div><h1>No rooms yet</h1><p>Create a private room or join one with an invite code.</p><button className="auth-submit" onClick={() => setCreateRoomOpen(true)}>Create a room</button></div></main>}</div>{createRoomOpen && <CreateRoomModal onCreated={created} onClose={() => setCreateRoomOpen(false)} />}{joinRoomOpen && <JoinRoomModal onJoined={joined} onClose={() => setJoinRoomOpen(false)} />}{profileOpen && user && <ProfileModal user={user} onSaved={profileSaved} onLogout={logout} onClose={() => setProfileOpen(false)} />}</>;
}
